...
async function fetchSupabaseApi(config, path, method = 'GET', body) {
    try {
...
export async function handleSupabaseRoutes(req, res, url) {
    ...
    try {
        ...
        if (req.method === 'GET' && url.pathname === '/codex-api/supabase/schemas') {
            const config = readConfig();
            if (!config) {
                setJson(res, 400, { error: 'Not configured' });
                return true;
            }
        ...
        if (req.method === 'POST' && url.pathname === '/codex-api/supabase/table-data') {
            ...
            if (!table) {
                setJson(res, 400, { error: 'table is required' });
                return true;
            }
            try {
                const client = getSupabaseClient(config);
        ...
        if (req.method === 'POST' && url.pathname === '/codex-api/supabase/sql') {
            ...
            if (!query) {
                setJson(res, 400, { error: 'query is required' });
                return true;
            }
        ...
        if (req.method === 'POST' && url.pathname === '/codex-api/supabase/storage/files') {
            ...
            if (!bucket) {
                setJson(res, 400, { error: 'bucket is required' });
                return true;
            }
            try {
                const client = getSupabaseClient(config);
